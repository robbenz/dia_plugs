...
class WC_Authorize_Net_CIM_API_Request extends XMLWriter {
	...
	public function get_create_customer_profile_transaction_request_xml( $order ) {
		...
		// @see set_duplicate_window() for explanation of duplicate window nonsense
		$this->startElement( 'extraOptions' );
			$this->writeCdata( 'x_solution_id=A1000065' . ( ! empty( $order->payment->cvv ) ? '&x_duplicate_window=0' : '' ) );
		$this->endElement();
	...
	public function get_transaction_request_xml( $order ) {
		...
		// tracking ID
		$this->startElement( 'solution' );

		$this->writeElement( 'id', 'A1000065' );
	...
	private function add_line_items( $order ) {
		...
		foreach ( $order->get_items() as $item_id => $item ) {
			...
			$line_items[] = array(
				...
				'description' => htmlentities( trim( is_object( $product ) ? $product->get_sku() : '') . ' ' . $meta, ENT_QUOTES, 'UTF-8', false ),
				'quantity'    => $item['qty'],
				'unit_price'  => SV_WC_Helper::number_format( isset( $item['recurring_line_total'] ) ? (float) $item['recurring_line_total'] : $order->get_item_total( $item ) ),
			);
		}
	...
	private function set_duplicate_window( $transaction_type ) {
		...
		if ( 'cim' === $transaction_type ) {
			...
			// now handled in transaction call so solution ID can be added
