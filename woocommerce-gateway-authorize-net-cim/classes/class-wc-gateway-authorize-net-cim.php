...
class WC_Gateway_Authorize_Net_CIM extends WC_Payment_Gateway {
	...
	public function validate_fields() {
		...
		if ( 'yes' === $this->require_cvv ) {
			...
			// check security code
			if ( empty( $cvv ) ) {
				SV_WC_Helper::wc_add_notice( __( 'Card security code is missing', WC_Authorize_Net_CIM::TEXT_DOMAIN ), 'error' );
				$is_valid = false;
			}

			// digit check
			if ( ! ctype_digit( $cvv ) ) {
				SV_WC_Helper::wc_add_notice( __( 'Card security code is invalid (only digits are allowed)', WC_Authorize_Net_CIM::TEXT_DOMAIN ), 'error' );
				$is_valid = false;
			}

			// length check
			if ( strlen( $cvv ) < 3 || strlen( $cvv ) > 4 ) {
				SV_WC_Helper::wc_add_notice( __( 'Card security code is invalid (must be 3 or 4 digits)', WC_Authorize_Net_CIM::TEXT_DOMAIN ), 'error' );
				$is_valid = false;
			}
		}
		...
		if ( ! ctype_digit( $expiration_month ) || ! ctype_digit( $expiration_year ) ||
		  ...
		  $expiration_year > $current_year + 20
			...
			SV_WC_Helper::wc_add_notice( __( 'Card expiration date is invalid', WC_Authorize_Net_CIM::TEXT_DOMAIN ), 'error' );
			$is_valid = false;
		}
		...
		if ( empty( $card_number ) || ! ctype_digit( $card_number ) || ! $this->luhn_check( $card_number ) ) {
			SV_WC_Helper::wc_add_notice( __( 'Card number is invalid', WC_Authorize_Net_CIM::TEXT_DOMAIN ), 'error' );
			$is_valid = false;
		}
	...
	protected function mark_order_as_failed( $order, $error_message ) {
		...
		SV_WC_Helper::wc_add_notice( __( 'An error occurred, please try again or try an alternate form of payment.', WC_Authorize_Net_CIM::TEXT_DOMAIN ), 'error' );
	}
	...
	public function get_api() {
		...
		// Authorize.net API
		require_once( wc_authorize_net_cim()->get_plugin_path() . '/classes/class-wc-authorize-net-cim-api.php' );
