...
class WC_Gateway_Authorize_Net_CIM_eCheck extends WC_Gateway_Authorize_Net_CIM {
	...
	public function validate_fields() {
		...
		if ( ! ctype_digit( $routing_number ) ) {
			SV_WC_Helper::wc_add_notice( __( 'Routing Number is invalid (only digits are allowed)', WC_Authorize_Net_CIM::TEXT_DOMAIN ), 'error' );
			$is_valid = false;
		}

		// routing number length check
		if ( 9 != strlen( $routing_number ) ) {
			SV_WC_Helper::wc_add_notice( __( 'Routing number is invalid (must be 9 digits)', WC_Authorize_Net_CIM::TEXT_DOMAIN ), 'error' );
			$is_valid = false;
		}

		// account number digit check
		if ( ! ctype_digit( $account_number ) ) {
			SV_WC_Helper::wc_add_notice( __( 'Account Number is invalid (only digits are allowed)', WC_Authorize_Net_CIM::TEXT_DOMAIN ), 'error' );
			$is_valid = false;
		}

		// account number length check
		if ( strlen( $account_number ) < 5 || strlen( $account_number ) > 17 ) {
			SV_WC_Helper::wc_add_notice( __( 'Account number is invalid (must be between 5 and 17 digits)', WC_Authorize_Net_CIM::TEXT_DOMAIN ), 'error' );
			$is_valid = false;
		}
