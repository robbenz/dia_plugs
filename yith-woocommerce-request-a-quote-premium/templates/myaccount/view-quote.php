...
if( ! $order ) {
	_e( 'This Quote doesn\'t exists.', 'yith-woocommerce-request-a-quote' );
...
if ( $order->get_status() == 'ywraq-new' ) {
	...
	if ( catalog_mode_plugin_enabled() ) {
		foreach ( $order->get_items() as $item_id => $item ) {
			...
			$hide_price = apply_filters( 'yith_ywraq_hide_price_template', WC()->cart->get_product_subtotal( $_product, $item['qty'] ), $_product->id );
			if ( $hide_price == '' ) {
				$show_price = false;
			}
		}
...
if ( in_array( $order->get_status(), array( 'ywraq-pending' , 'pending' ) ) ): ?>
	<p class="ywraq-buttons">
		...
		<?php if ( get_option( 'ywraq_show_accept_link' ) != 'no' ) : ?><a class="ywraq-big-button ywraq-accept" href="<?php echo esc_url( add_query_arg( array( 'request_quote' => $order_id, 'status' => 'accepted', 'raq_nonce' => ywraq_get_token( 'accept-request-quote', $order_id, $user_email ) ), YITH_Request_Quote()->get_raq_page_url() ) ) ?>"><?php echo $accept_button_text ?></a><?php endif ?>
		<?php if ( get_option( 'ywraq_show_reject_link' ) != 'no' ) : ?> <a class="ywraq-big-button ywraq-reject" href="<?php echo esc_url( add_query_arg( array( 'request_quote' => $order_id, 'status' => 'rejected', 'raq_nonce' => ywraq_get_token( 'reject-request-quote', $order_id, $user_email ) ), YITH_Request_Quote()->get_raq_page_url() ) ) ?>"><?php ywraq_get_label( 'reject', true ) ?></a><?php endif ?>
	</p>

<?php elseif( $order->get_status() == 'ywraq-accepted' ): ?>
	<p class="ywraq-buttons">
		<?php if ( $print_button_pdf ) { ?><a class="ywraq-big-button ywraq-pdf-file" href="<?php echo esc_url( $pdf_file ) ?>" target="_blank"><?php _e( 'Download PDF', 'yith-woocommerce-request-a-quote' ) ?></a><?php } ?>
		<?php if ( get_option( 'ywraq_show_accept_link' ) != 'no' && YITH_Request_Quote()->enabled_checkout() ) : ?><a class="ywraq-big-button ywraq-accept" href="<?php echo esc_url( add_query_arg( array( 'request_quote' => $order_id, 'status' => 'accepted', 'raq_nonce' => ywraq_get_token( 'accept-request-quote', $order_id, $user_email ) ), YITH_Request_Quote()->get_raq_page_url() ) ) ?>"><?php echo $accept_button_text ?></a><?php endif ?>
		<?php if ( get_option( 'ywraq_show_reject_link' ) != 'no' ) : ?><a class="ywraq-big-button ywraq-reject" href="<?php echo esc_url( add_query_arg( array( 'request_quote' => $order_id, 'status' => 'rejected', 'raq_nonce' => ywraq_get_token( 'reject-request-quote', $order_id, $user_email ) ), YITH_Request_Quote()->get_raq_page_url() ) ) ?>"><?php ywraq_get_label( 'reject', true ) ?></a><?php endif ?>
	</p>
<?php else: ?>
