        ...
        */
        public function __construct() {
           ...
           if( ! catalog_mode_plugin_enabled() ){
            ...
            add_filter( 'yith_ywraq_before_print_widget', array($this, 'must_be_showed') );
            add_filter( 'yith_ywraq_before_print_my_account_my_quotes', array($this, 'must_be_showed') );
            add_filter( 'yith_ywraq_before_print_raq_page', array($this, 'raq_page_check_user') );
            add_filter( 'yith_ywraq_raq_page_deniend_access', array($this, 'raq_page_deniend_access') );
        ...
        public function add_button_shop() {
            ...
            if ( $show_button != 'yes' || ! in_array( $product->product_type, $type_in_loop ) ) {
                return false;
            }
        ...
        public function show_product_price( $price, $product_id = false ) {
            ...
            if ( $hide_price && !catalog_mode_plugin_enabled() && ! ( defined( 'DOING_AJAX' ) && DOING_AJAX )  ) {
                ob_start();
        ...
        public function must_be_showed(){
           global $product;
            ...
            if( ! $this->check_user_type() || ( get_option('ywraq_show_btn_only_out_of_stock') == 'yes' && $product &&  $product->is_in_stock() ) ){
                return false;
            }
        ...
        public function check_user_type(){
            ...
            if ( is_user_logged_in() && ( $user_type == 'customers' || $user_type == 'all' ) ) {
                ...
                if ( $user_type == 'guests' || $user_type == 'all' ) {
                    return true;
                }
            }
         ...
         * @return string|void
         */
        public function raq_page_deniend_access( $message ){
            $user_type = get_option('ywraq_user_type');
        ...
        public function generate_pdf( $order_id ) {
            ...
            wc_get_template( 'quote.php', array( 'order_id'=> $order_id ), YITH_YWRAQ_TEMPLATE_PATH . '/pdf/', YITH_YWRAQ_TEMPLATE_PATH . '/pdf/' );

            $html = ob_get_contents();
            ob_end_clean();

            require_once( YITH_YWRAQ_DOMPDF_DIR . "dompdf_config.inc.php" );
        ...
        public function get_pdf_file_url( $order_id) {
            ...
            return YITH_YWRAQ_SAVE_QUOTE_URL. $path .'quote_'.$order_id.'.pdf';
        }
        ...
        public function get_pdf_file_path( $order_id, $delete_file = false) {
            $path = $this->create_storing_folder($order_id);
            $file = YITH_YWRAQ_DOCUMENT_SAVE_DIR. $path .'quote_'.$order_id.'.pdf';
...
//            /* Create folders for storing documents */
            ...
            $date           = getdate( strtotime($order->order_date) ) ;
        ...
        public function pdf_content( $order_id ){
            $order = wc_get_order($order_id);
            wc_get_template( 'quote-table.php', array( 'order'=>$order ), YITH_YWRAQ_TEMPLATE_PATH . '/pdf/', YITH_YWRAQ_TEMPLATE_PATH . '/pdf/' );
        }
        ...
        public function pdf_header( $order_id ){
            $order = wc_get_order($order_id);
            wc_get_template( 'quote-header.php', array( 'order'=>$order ), YITH_YWRAQ_TEMPLATE_PATH . '/pdf/', YITH_YWRAQ_TEMPLATE_PATH . '/pdf/' );
        }
        ...
        public function pdf_footer( $order_id ){
            ...
            $show_pagination = get_option( 'pdf_pagination' );
            wc_get_template( 'quote-footer.php', array( 'footer' => $footer_content, 'pagination' => $show_pagination, 'order_id' => $order_id ), YITH_YWRAQ_TEMPLATE_PATH . '/pdf/', YITH_YWRAQ_TEMPLATE_PATH . '/pdf/' );
        }
