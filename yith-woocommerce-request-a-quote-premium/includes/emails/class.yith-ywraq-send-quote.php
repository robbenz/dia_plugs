    ...
    class YITH_YWRAQ_Send_Quote extends WC_Email {
        ...
        public function trigger( $order_id ) {
            ...
            if ( $order_id ) {
                ...
                $this->raq['customer_message'] = get_post_meta( $this->order->id, 'ywraq_customer_message', true );
                $this->raq['admin_message']    = nl2br( get_post_meta( $this->order->id, '_ywcm_request_response', true ) );
                $this->raq['user_email']       = get_post_meta( $this->order->id, 'ywraq_customer_email', true );
                $this->raq['user_name']        = get_post_meta( $this->order->id, 'ywraq_customer_name', true );
                ...
                $this->raq['order-date']       = date_i18n( wc_date_format(), strtotime( $this->order->order_date ) );
                $this->raq['order-number']     = $this->order->get_order_number();
        ...
        public function get_attachments(){
            ...
            if ( '' != $optional_upload = get_post_meta( $this->order_id, '_ywraq_optional_attachment', true ) ) {
