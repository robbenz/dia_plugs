...
class YITH_YWRAQ_Order_Request {
    ...
    public function __construct() {
        ...
        $this->_data['raq_order_status'] = array( 'wc-ywraq-new', 'wc-ywraq-pending', 'wc-ywraq-expired', 'wc-ywraq-rejected', 'wc-ywraq-accepted' );
    ...
    public function create_order( $raq ) {
        ...
        // Store tax rows
        foreach ( array_keys( $new_cart->taxes + $new_cart->shipping_taxes ) as $tax_rate_id ) {
            if ( $tax_rate_id && ! $order->add_tax( $tax_rate_id, $new_cart->get_tax_amount( $tax_rate_id ), $new_cart->get_shipping_tax_amount( $tax_rate_id ) ) && apply_filters( 'woocommerce_cart_remove_taxes_zero_rate_id', 'zero-rated' ) !== $tax_rate_id ) {
                throw new Exception( sprintf( __( 'Error %d: Unable to create order. Please try again.', 'yith-woocommerce-request-a-quote' ), 528 ) );
            }
        }
    ...
    public function add_order_meta( $order, $raq ) {
       ...
       if ( isset( $raq['other_email_fields'] ) ) {
            add_post_meta( $order->id, 'ywraq_customer_other_email_fields', $raq['other_email_fields'] );
        }
...
//            $cart_item_data = apply_filters( 'ywraq_add_to_cart', $cart_item_data, $item );
    ...
    public function ajax_accept_order() {
        if ( ! isset( $_POST['order_id'] ) ) {
            ...
            if ( $accepted_page_id == $checkout_page_id ) {
                ...
                $result = array(
                    'result'  => 1,
                    'rqa_url' => get_permalink( $accepted_page_id ),
                );
            }
     ...
     * @param $order_id
     */
    function accept_reject_order( $status, $order_id ){
        ...
        if ( $status == 'accepted' && ywraq_verify_token( $_GET['raq_nonce'], 'accept-request-quote', $order_id, $user_email ) ) {
            ...
            if ( in_array( $current_status, array( 'ywraq-pending', 'pending', 'ywraq-accepted') ) ) {
                if ( YITH_Request_Quote()->enabled_checkout() ) {
                    ...
                    wp_safe_redirect( get_permalink( get_option( 'ywraq_page_accepted' ) ) );
                    exit;
                }
            } else {
    ...
    public function set_package_rates( $rates ) {
        ...
        if ( ! empty( $shipping_items ) ) {
            foreach ( $shipping_items as $shipping_item ) {
                if ( isset( $rates[ $shipping_item['method_id'] ] ) ) {
                    $rates[ $shipping_item['method_id'] ]->cost= $shipping_item['cost'];
                }
            }
        }
